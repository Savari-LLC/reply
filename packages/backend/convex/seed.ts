...
export const SAMPLE_DATASETS: Array<{
  key: SampleDataset;
  name: string;
  emailAddress: string;
  displayName: string;
}> = [
  {
    key: "sales",
    name: "Sales sample",
    emailAddress: "sales@sample.reply.test",
    displayName: "Sample data — Sales",
  },
  {
    key: "accounts",
    name: "Accounts sample",
    emailAddress: "accounts@sample.reply.test",
    displayName: "Sample data — Accounts",
  },
  {
    key: "support",
    name: "Support sample",
    emailAddress: "support@sample.reply.test",
    displayName: "Sample data — Support",
  },
];
...
/**
 * Seeds one sample dataset into an inbox through a freshly created demo
 * channel. The caller is responsible for authorization; `actorId` receives
 * read state matching the dataset's unread story.
 */
export async function connectSampleData(
  ctx: MutationCtx,
  args: {
    workspaceId: Id<"workspaces">;
    inboxId: Id<"inboxes">;
  ...
  const { workspaceId, inboxId, actorId, dataset } = args;
  const datasetMeta = SAMPLE_DATASETS.find((entry) => entry.key === dataset);
  if (!datasetMeta) throw new Error("Unknown sample dataset");
  ...
  const channelId = await ctx.db.insert("channels", {
    ...
    inboxId,
    provider: "demo",
    emailAddress: datasetMeta.emailAddress,
    displayName: datasetMeta.displayName,
    status: "connected",
  });
